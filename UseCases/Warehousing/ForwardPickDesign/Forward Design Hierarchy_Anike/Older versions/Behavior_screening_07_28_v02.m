...
for b = 1:p % points in latin hypercube design
    ...
    for m = 1:50
        ...
        for o = 1:tours
            ...
            travel_time(o) = time + sd_bh(b).penalty * length(stops);
        end
        
        if tours * sd_bh(b).PickEquipment.capacity < 100
            % Batching (Monte Carlo Sampling)
            stops = sd_bh(b).GenerateOrders(100-(length(travel_time)*sd_bh(b).PickEquipment.capacity));
            ...
            travel_time(end+1) = time + sd_bh(b).penalty * length(stops);  
        end
    ...
    %% Calculate metrics (time to clear out 100 orders, mean time per tour, variable and capital cost)
    mn_max_time(b) = mean(max_time);
    mn_time(b) = mean(mn_travel_time);
    
    var_cost(b) = mn_max_time(b)/3600 * sd_bh(b).PickEquipment.variable_cost * sd_bh(b).fleet_size;
    cap_cost(b) = sd_bh(b).fleet_size * sd_bh(b).PickEquipment.fixed_cost + sd_bh(b).StorageEquipment.fixed_cost;
    
    sd_bh(b).time100 = mn_max_time(b);
    sd_bh(b).time_per_tour = mn_time(b);
    sd_bh(b).var_cost = var_cost(b);
    sd_bh(b).cap_cost = cap_cost(b);
    
    b
end

%% Find Pareto front
X = [mn_max_time,mn_time,var_cost,cap_cost];
front = paretoGroup(X);
...
figure
scatter3(var_cost,cap_cost,mn_max_time,ones(p,1)*36,color,'filled')
...
figure
scatter3(var_cost,cap_cost,mn_time,ones(p,1)*36,color,'filled')
title('Screening Forward Behavior (capacity, speed, fleet size)')
xlabel('Variable Cost')
ylabel('Capital Cost')
