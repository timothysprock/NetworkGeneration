...
D = dijk(A, PD, StorageNetworkNodes(:,1));
N = length(StorageNetworkNodes(:,1));
if N > 200
    delta_ij = zeros(N);
        parfor i= 1:N
            delta_ij(i, :) = dijk(A, StorageNetworkNodes(i,1), StorageNetworkNodes(:,1));
        end
else
    delta_ij = dijk(A, StorageNetworkNodes(:,1), StorageNetworkNodes(:,1));
end
toc
