...
for e = 1:length(seqSet)
    ...
    for sku_percent = 0.1:0.1:1;
        ...
        space = forward_length * forward_width;
        
        if c == 1
            mn = [space,sku_percent,mean(travel_time)];
            v = [space,sku_percent,var(travel_time)];
        else
            mn = [mn; [space,sku_percent,mean(travel_time)]];
            v = [v; [space,sku_percent,var(travel_time)]];
        end
