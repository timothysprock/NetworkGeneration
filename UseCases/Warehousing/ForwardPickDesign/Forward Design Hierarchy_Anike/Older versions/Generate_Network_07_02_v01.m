...
% Vertical edges
for aisle_count = 0:sd.aisles-1
    logLeftAisleBaySet = NodeSetList(:,2) == 0.5*sd.StorageEquipment.bay_width+aisle_count*sd.aisle_module_width; %all the bays on the left side of an aisle
    logRightAisleBaySet = NodeSetList(:,2) == 1.5*sd.StorageEquipment.bay_width+sd.aisle_width+aisle_count*sd.aisle_module_width; %all the bays on the right side of an aisle

    LeftAisleBaySet = sortrows(NodeSetList(logLeftAisleBaySet,:),3);
    RightAisleBaySet = sortrows(NodeSetList(logRightAisleBaySet,:),3);
    
    for node_count = 1:ceil(sd.bays/sd.aisles/sd.StorageEquipment.storage_height/2)
        
        for height_count = 1:sd.StorageEquipment.storage_height-1
            i = height_count*node_count;
            EdgeSetList(E,:) = [E, LeftAisleBaySet(i,1), LeftAisleBaySet(i+1,1), sd.StorageEquipment.bay_height];
            EdgeSetList(E+1,:) = [E+1, LeftAisleBaySet(i+1,1), LeftAisleBaySet(i,1), sd.StorageEquipment.bay_height];
            EdgeSetList(E+2,:) = [E+2, RightAisleBaySet(i,1), RightAisleBaySet(i+1,1), sd.StorageEquipment.bay_height];
            EdgeSetList(E+3,:) = [E+3, RightAisleBaySet(i+1,1), RightAisleBaySet(i,1), sd.StorageEquipment.bay_height];
            E=E+4;
        end
    end
end
...
gplot32(sd.StorageNetwork.EdgeSetAdjList, [UnitedNodeSetList(:,2), UnitedNodeSetList(:,3), UnitedNodeSetList(:,4)])
hold off
