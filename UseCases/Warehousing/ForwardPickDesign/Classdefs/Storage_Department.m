    ...
    methods
        ...
        %% Initialize Layout Procedure
            ...
            for aisle_count = 0:SD.aisles-1
                ...
                NodeSetList(N,:) = [N, 0.5*SD.aisle_width+aisle_count*SD.aisle_width, 0.25*SD.aisle_width, 0];
                Cross_Aisle_Set(2*aisle_count+1) = N;

                last_xyz = [0.5*SD.aisle_width+aisle_count*SD.aisle_width, 0.25*SD.aisle_width, 0];
                ...
                for node_count = 1:SD.k
                    ...
                    NodeSetList(N,:) = [N, 0.5*SD.aisle_width+aisle_count*SD.aisle_width, node_count*(aislelength/SD.k)+ 0.25*SD.aisle_width,0];

                    distance = max(1e-6,sqrt(sum((last_xyz - [0.5*SD.aisle_width+aisle_count*SD.aisle_width, node_count*(aislelength/SD.k) + 0.25*SD.aisle_width,0]).^2)));
                    EdgeSetList(E-2, 4) = distance;
                    EdgeSetList(E-1, 4) = distance;

                    last_xyz = [0.5*SD.aisle_width+aisle_count*SD.aisle_width, node_count*(aislelength/SD.k) + 0.25*SD.aisle_width,0];
                    EdgeSetList(E,:) = [E, N, N+1, 0];
                    EdgeSetList(E+1,:) = [E+1, N+1, N, 0];
                ...
                NodeSetList(N,:) = [N, 0.5*SD.aisle_width+aisle_count*SD.aisle_width,node_count*(aislelength/SD.k) + 0.5*SD.aisle_width,0];
                Cross_Aisle_Set(2*aisle_count+2) = N;

                distance = max(1e-6,sqrt(sum((last_xyz - [0.5*SD.aisle_width+aisle_count*SD.aisle_width, node_count*(aislelength/SD.k) + 0.5*SD.aisle_width,0]).^2)));
                EdgeSetList(E-2, 4) = distance;
                EdgeSetList(E-1, 4) = distance;
